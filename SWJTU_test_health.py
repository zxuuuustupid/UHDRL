...
def main():
    ...
    if os.path.exists(
        ...
        # Step 3: build graph
    accuracy_list = [[0]*5  for _ in range(1)]
        ...
        for ten_epoches in range(1, 11):
            ...
            for i in range(TEST_EPISODE):
                ...
                metatest_character_folders1 = [f'../CWT-XJT/test/health/WC{num_wc}',
                                               f'../CWT-XJT/test/anomaly/5-RollerWearing/WC{num_wc}']
                ...
                if test_labels[1] == 0:
                    ...
                    recall_reward = [1 if predict_labels[0] == test_labels[0] else 0]
                    recall_rewards += np.sum(recall_reward)
                # if test_labels[0] == 1:
                #     recall_times = recall_times + 1
                #     recall_reward = [1 if predict_labels[1] == test_labels[1] else 0]
                #     recall_rewards += np.sum(recall_reward)
            recall = recall_rewards / 1.0 / CLASS_NUM / recall_times
            total_recall = total_recall + recall
        accuracy_list[0][num_wc - 1] = total_recall / 10.0
...
if __name__ == '__main__':
    ...
    # 构建目标文件路径
    file_path_acc = os.path.join('test_result', 'SWJTU','mixed','acc_health.csv')
