...
clc
% 基础路径
base_path = 'F:\Project\TripletLoss\BJTU-RAO Bogie Datasets\Data\BJTU_RAO_Bogie_Datasets\M0_G8_LA0_RA0\';
...
for sample = 1:9
    ...
    % 文件名和完整路径
    file_name = sprintf('Sample_%d\\data_gearbox_M0_G8_LA0_RA0_%dHz_%s.csv', ...
                        sample, frequencies(freq_index), forces{force_index});
    full_path = fullfile(base_path, file_name);
    ...
    % 动态保存路径和文件名
    name = sprintf('M0_G8_LA0_RA0_gearbox_CH18');
    save_path = sprintf('CWT3-1000\gearbox\test\G8\anomaly\WC%d',sample);
