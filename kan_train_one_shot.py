...
parser.add_argument("-b", "--batch_num_per_class", type=int, default=4)  #此处原来是19但我显存不够了
...
def main():
    ...
    # relation_network = KAN([28 * 28 * 512,  2])
        ...
        if (episode + 1) % 100 == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                relation_pairs = torch.cat((sample_features_ext, test_features_ext), 2).view(-1, FEATURE_DIM * 2 *
                                                                                             2 ,28 * 28)
                relations2 = relation_network(relation_pairs)
                relations2 = relations2.view(2, 8 * 512)
                relations2 = kan(relations2)
                relations = relations2.view(-1, CLASS_NUM)
