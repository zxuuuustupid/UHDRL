...
# faulttype = 'gearboxtype12_1000'
faulttype = 'rightaxleboxtype113'
root_path = '../Test_Data/Mixed_fault/'
test_result_sim = './test_result/mixed_similar_score/'
test_result_acc = './test_result/mixed_acc_back/'
...
def main():
    ...
    rightaxlebox_feature_encoder = CNNEncoder1.rsnet()  # 特征提取
    rightaxlebox_relation_network = KAN([28 * 28, 128, 8])  # 定义关系网络
    rightaxlebox_relation_network_2 = KAN([8 * 512, 512, 32, 2])
    ...
    rightaxlebox_feature_encoder.cuda(GPU)
    rightaxlebox_relation_network.cuda(GPU)
    rightaxlebox_relation_network_2.cuda(GPU)
    ...
    if os.path.exists(
            str("./models/rightaxlebox_feature_encoder_" + str(CLASS_NUM) + "way_" + str(
                SAMPLE_NUM_PER_CLASS) + "shot.pkl")):
        rightaxlebox_feature_encoder.load_state_dict(torch.load(
            str("./models/rightaxlebox_feature_encoder_" + str(CLASS_NUM) + "way_" + str(
                SAMPLE_NUM_PER_CLASS) + "shot.pkl")))
        print("load rightaxlebox feature encoder success")
    if os.path.exists(
            str("./models/rightaxlebox_relation_network_" + str(CLASS_NUM) + "way_" + str(
                SAMPLE_NUM_PER_CLASS) + "shot.pkl")):
        rightaxlebox_relation_network.load_state_dict(torch.load(
            str("./models/rightaxlebox_relation_network_" + str(CLASS_NUM) + "way_" + str(
                SAMPLE_NUM_PER_CLASS) + "shot.pkl")))
        print("load rightaxlebox relation network success")
    if os.path.exists(
            str("./models/rightaxlebox_relation_network_2" + str(CLASS_NUM) + "way_" + str(
                SAMPLE_NUM_PER_CLASS) + "shot.pkl")):
        rightaxlebox_relation_network_2.load_state_dict(torch.load(
            str("./models/rightaxlebox_relation_network_2" + str(CLASS_NUM) + "way_" + str(
                SAMPLE_NUM_PER_CLASS) + "shot.pkl")))
        print("load rightaxlebox relation network2 success")

    print("Testing...motor type9 ")
    acc_back = []
    for episode in range(10):
        ...
        with torch.no_grad():
            for i in range(TEST_EPISODE):
                ...
                metatest_character_folders1 = [root_path + 'rightaxlebox/test/health',
                                               root_path + 'rightaxlebox/test/anomaly/anomalyTYPE113']  #
                metatrain_character_folders1 = [root_path + 'rightaxlebox/train/health',
                                                root_path + 'rightaxlebox/train/anomaly']
                ...
                sample_features = rightaxlebox_feature_encoder(Variable(sample_images).cuda(GPU))  # 5x64
                test_features = rightaxlebox_feature_encoder(Variable(test_images).cuda(GPU))  # 20x64
                ...
                relations1 = rightaxlebox_relation_network(relation_pairs)
                print(relations1.shape)
                relations1 = relations1.view(2, 8 * 512)
                print(relations1.shape)
                relations1 = rightaxlebox_relation_network_2(relations1)
                print(relations1.shape)
                ...
                for j in range(len(relations)):
                    ...
                    if relations[j][0] >= boundary:
                        bb[j] = 0
                        sim_score.append([test_labels[j].cpu().item(), relations[j][0].cpu().item()])
                        # scores_result.append(relations[j][0].cpu().item())  # 保存相似分数1.0
                        # # print(test_labels[j])# 保存预测结果
                ...
                if test_labels[0]==1:
                    back_reward=[1 if predict_labels[j] == test_labels[j] else 0 for j in range(CLASS_NUM)]
                    back_acc = np.sum(back_reward) / 1.0 / CLASS_NUM / SAMPLE_NUM_PER_CLASS
        ...
        # print(test_accuracy)
        acc_test_accuracy, std = mean_confidence_interval(back_accuracies)
        # print(acc_test_accuracy)
        acc_back.append([test_accuracy, acc_test_accuracy])

        # # print(len(sim_score))
        health, anomaly = [], []
        for i in range(len(sim_score)):
            if sim_score[i][0] == 1:
                anomaly.append(sim_score[i][1])
            if sim_score[i][0] == 0:
                health.append(sim_score[i][1])

        # print("test accuracy:", test_accuracy)
        # np.savetxt(test_result + 'Left' + '_ture_result.csv', np.array(ture_result), fmt='%.4f', delimiter=',')
        # np.savetxt(test_result_sim + faulttype + '_scores.csv', np.array(scores_result), fmt='%.4f', delimiter=',')
    ...
    # np.savetxt(test_result_sim + faulttype + '_health.csv', np.array(health), fmt='%.4f', delimiter=',')
    # np.savetxt(test_result_sim + faulttype + '_anomaly.csv', np.array(anomaly), fmt='%.4f', delimiter=',')
    # np.savetxt(test_result_acc + faulttype + '_acc_back.csv', np.array(acc_back), fmt='%.4f', delimiter=',')
