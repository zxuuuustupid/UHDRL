...
def main():
    ...
    if os.path.exists(
            str("./models/motor_WC4+WC5_feature_encoder_" + str(CLASS_NUM) + "way_" + str(
                SAMPLE_NUM_PER_CLASS) + "shot.pkl")):
    ...
    if os.path.exists(
            str("./models/motor_WC4+WC5_relation_network_" + str(CLASS_NUM) + "way_" + str(
                SAMPLE_NUM_PER_CLASS) + "shot.pkl")):
    ...
    if os.path.exists(
            str("./models/motor_WC4+WC5_relation_network_2" + str(CLASS_NUM) + "way_" + str(
                SAMPLE_NUM_PER_CLASS) + "shot.pkl")):
    ...
    # Step 3: build graph
    accuracy_list = [[0] * 2 for _ in range(4)]
    recall_list = [[0] * 2 for _ in range(4)]
    std_list = [[0] * 2 for _ in range(4)]
    for num_fault_type in range(1, 4 + 1):
        for num_wc in range(1, 2 + 1):
            total_acc = 0
            total_recall = 0
            acc_for_std_list = []
            for ten_epoches in range(1, 11):
                ...
                for i in range(TEST_EPISODE):
                    ...
                    # metatest_character_folders1 = [f'../CWT-1000/motor/test/health/WC{num_wc}',
                    #                                f'../CWT-1000/motor/test/LA{num_fault_type}/anomaly/WC{num_wc}']
                    # metatrain_character_folders1 = [f'../CWT-1000/motor/train/health/WC{num_wc}',
                    #                                 '../CWT-1000/motor/train/anomaly']
                    metatest_character_folders1 = [f'../CWT3-1000/motor/test/health/WC{num_wc}',
                                                   f'../CWT3-1000/motor/test/M{num_fault_type}/anomaly/WC{num_wc}']
                    metatrain_character_folders1 = [f'../CWT3-1000/motor/train/health/WC{num_wc}',
                    ...
                    sample_features = motor_feature_encoder(Variable(sample_images).cuda(GPU))  # 5x64
                    test_features = motor_feature_encoder(Variable(test_images).cuda(GPU))  # 20x64
                    ...
                    relations1 = motor_relation_network(relation_pairs)
                    relations1 = relations1.view(2, 8 * 512)
                    relations1 = motor_relation_network_2(relations1)
            ...
            print("FaultType:M", str(num_fault_type).ljust(2),
...
if __name__ == '__main__':
    ...
    # 构建目标文件路径
    file_path_std = os.path.join('test_result', 'motor', 'motor_std_WC4+WC5.csv')
    file_path_acc = os.path.join('test_result', 'motor', 'motor_acc_WC4+WC5.csv')
    file_path_recall = os.path.join('test_result', 'motor', 'motor_recall_WC4+WC5.csv')
    # 保存为 CSV 文件
