...
def main():
    ...
    # Step 3: build graph
    # TODO Test Here Many Times
    accuracy_list = [[0] * 4 for _ in range(4)]
    recall_list = [[0] * 4 for _ in range(4)]
    std_list = [[0] * 4 for _ in range(4)]
    for num_fault_type in range(1, 4 + 1):
        # TODO Test Here Many Times
        for num_wc in range(1, 4 + 1):
            total_acc = 0
            total_recall = 0
            acc_for_std_list = []
...
if __name__ == '__main__':
    ...
    # 构建目标文件路径
    # TODO Test Here Many Times
    file_path_std = os.path.join('test_result', 'motor', 'motor_WC4+WC5+WC6+WC7','motor_std_WC4+WC5+WC6+WC7.csv')
    file_path_acc = os.path.join('test_result', 'motor','motor_WC4+WC5+WC6+WC7', 'motor_acc_WC4+WC5+WC6+WC7.csv')
    file_path_recall = os.path.join('test_result', 'motor', 'motor_WC4+WC5+WC6+WC7','motor_recall_WC4+WC5+WC6+WC7.csv')
    # 保存为 CSV 文件
